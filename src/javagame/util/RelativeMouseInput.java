...
/**
 * At first it may seem obvious,
 * but the program only receives mouse events when the mouse is in the window.
 * Once the mouse leaves the window,the coordinates where the mouse 
 * pointer is are no longer valid in the application.
 * To make this worse,when in full screen mode,
 * the mouse simple stops when it reaches the edge of the screen and doesn't continue registering events.
 * Depending on the needs of the application,relative mouse movement may be needed.
 * @author 79001
 *
 */
public class RelativeMouseInput implements MouseListener,MouseMotionListener,MouseWheelListener{
    ...
    private Robot robot;
    /**
     * The Swing component that listens for mouse events is include to calculate the center of the window 
     * and convert from relative window coordinates to absolute screen coordinates.
     * If the mouse cursor is always in the center of the window,
     * then it can never leave,
     * and the window will always receive mouse events.
     */
...
//        Not needed
    ...
    /**
     * A flag is added to the RelativeMouseInput class to allow both relative and absolute mouse movement.
     * It is possible that a game will need both absolute and relative mouse modes at some point,
     * so the class allows switching this at runtime.
...
//            自己写的方法
    ...
    /**
     * The center of the window is computed at runtime so even if the window is resized,
     * the mouse will stay in the center.
     * The relative center point must be converted to absolute screen coordinates.
     * The upper left pixel in a window is (0,0) no matter where the window is located on the desktop.
     * If moving the window around the desktop changed the value of the upper left pixel,
     * graphics programming would be very difficult.
     * Although it makes drawing easier by using relative pixel values,
     * positioning the mouse to the 
     * relative center of the window does not take into account the location of the window 
     * and could place the mouse cursor so far away from the window that it would stop receiving mouse events.
     * Converting to screen coordinates using the SwingUtilities class solves this problem.
     * @return
     */
